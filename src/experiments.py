...
def construct_model(args):
    ...
    dataname = args.data_dir.split("/")[-1]
    ...
    if args.model in ['point', 'point.gc']:
        ...
        if fn_model == 'complex':
            if dataname == "WN9IMG":
                ling_path = os.path.join(args.data_dir, "pre_train/matrix_wn_ling.npy")
                visual_path = os.path.join(args.data_dir, "pre_train/matrix_wn_visual.npy")
                ling_f, visual_f = torch.tensor(np.load(ling_path)), torch.tensor(np.load(visual_path))
            
            elif dataname == "FBIMG":
                ling_path = os.path.join(args.data_dir, "pre_train/matrix_fb_ling.npy")
                visual_path = os.path.join(args.data_dir, "pre_train/matrix_fb_visual.npy")
                ling_f, visual_f = torch.tensor(np.load(ling_path)), torch.tensor(np.load(visual_path))
            else:
                ling_f, visual_f = None, None
            fn = ComplEx(fn_args, ling_f, visual_f)
            fn_kg = KnowledgeGraph(fn_args, ling_f, visual_f)
        ...
        fn = ComplEx(args,ling_f, visual_f) 
