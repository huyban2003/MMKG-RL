...
class ComplEx(nn.Module):
    def __init__(self, args, ling_f, visual_f):
        ...
        self.ling_vec = ling_f.to(torch.float32)
        self.img_vec = visual_f.to(torch.float32)

        self.ling_dimension = ling_f.shape[-1]
        self.img_dimension = visual_f.shape[-1]

        self.mats_img = nn.Parameter(torch.Tensor(self.img_dimension, self.entity_dim *2 ), requires_grad=True)
        nn.init.xavier_uniform_(self.mats_img)
        self.mats_ling = nn.Parameter(torch.Tensor(self.ling_dimension, self.entity_dim * 2), requires_grad=True)
        nn.init.xavier_uniform_(self.mats_ling)

        # self.ffn = nn.Sequential(
        #     nn.Linear(2 * self.entity_dim, 4 * self.entity_dim),
        #     nn.LeakyReLU(),
        #     nn.BatchNorm1d(4 * self.entity_dim),
        #     nn.Dropout(0.1),                  
        #     nn.Linear(4 * self.entity_dim, 4 * self.entity_dim),
        #     nn.LeakyReLU(),
        #     nn.BatchNorm1d(4 * self.entity_dim),
        #     nn.Dropout(0.1),                   
        #     nn.Linear(4 * self.entity_dim, 4 * self.entity_dim),
        ...
        #     nn.BatchNorm1d(4 * self.entity_dim),
        #     nn.Dropout(0.1),                  
        #     nn.Linear(4 * self.entity_dim, 2 * self.entity_dim),
        #     nn.BatchNorm1d(2 * self.entity_dim)
        # )

        # self.layer_norm = nn.LayerNorm(self.entity_dim * 2)
    ...
    def forward(self, e1, r, kg):
        ...
        img_embeddings = self.img_vec.to('cuda').mm(self.mats_img.to('cuda'))
        ling_embeddings = self.ling_vec.to('cuda').mm(self.mats_ling.to('cuda'))
